...
for k2_ind = 1 : numel(ky)
    ...
    if( k1_ind ~= 1 || k2_ind ~= 1 ) % Origin, different solution, be mindful of dividing by k vector as well!
        ...
        [eigenvec, eigenval] = eig(vpa_mat);
        eigenvec = double(eigenvec);
        eigenval = diag(double(eigenval));
...
for k2_ind = 1 : Ny
    if( k1_ind ~= 1 || k2_ind ~= 1 ) % Because we divide by K^2 to make matrices better
        ...
        vpa_mat = vpa(bc_mat);
        strain_bc_mats_inv(k1_ind,k2_ind,:,:) = double(inv(vpa_mat));
    end
end
end

strain_bc_mat_inv_korigin = double( inv( vpa([ h_sub                1   0                   0   0                   0 ; ...
                                               0                    0   h_sub               1   0                   0 ; ...
                                               0                    0   0                   0   h_sub               1 ; ...
