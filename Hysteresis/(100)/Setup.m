...
% Do not need E applied vars
% Do not need PATH Var
rng(100)
...
Ferro_size = 4;
Ferro_pct = ones(Nx,Ny,Nz);
Ferro_pct(:,:,[1:Nz] > Ferro_size + interface_index) = 0;

Dielectric_pct = ones(Nx,Ny,Nz);
Dielectric_pct(:,:,[1:Nz] <= Ferro_size + interface_index) = 0;
...
a1_T = @(T) a1_Ferro(T) * Ferro_pct + a1_Dielectric(T) * Dielectric_pct;
a1 = a1_T(Temperature);
a11 = a11_Ferro * Ferro_pct + a11_Dielectric * Dielectric_pct;
a12 = 0;
a111 = 0;
a112 = 0;
a123 = 0;
...
P = [-2 : 0.01 : 2];
F_Ferro = a1_Ferro(Temperature) * P.^2 + a11_Ferro * P.^4;
F_Dielectric = a1_Dielectric(Temperature) * P.^2;
plot(P,F_Ferro)
hold on
plot(P,F_Ferro + F_Dielectric);
% 
% P_s = sqrt( -a1_Ferro(Temperature) / a11_Ferro );
% barrier = -a1_Ferro(Temperature)^2 / (2 * a11_Ferro);
%% Simulation Size
l_0 = 0.5e-9;
...
%% Electrical
k_electric_11 = 50; k_electric_22 = k_electric_11; k_electric_33 = k_electric_11;